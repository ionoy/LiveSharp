...
namespace LiveSharp.Support.XamarinForms
{
    class InstanceInspector
    {
        ...
        public InstanceInspector(INotifyPropertyChanged inpc, ILiveSharpLogger logger)
        {
            _type = inpc.GetType();

            var properties = GetAllProperties(inpc);

            Properties.AddRange(properties.Select(pi => new PropertyInspector(pi, inpc, logger)));
        }
